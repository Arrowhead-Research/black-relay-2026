...
How the existing Helsinki CPX32 and its independent Primary IPv4 are adopted,
along with the production firewall, DNS records, and protected BX11. Adoption is
complete; this is the reference for re-importing state, recovering from an
interrupted run, and the single-writer discipline every apply still follows.

No command here authorizes deleting, rebuilding, or replacing a protected
resource. Stop if any plan contains a `delete` action.
...
initial password and state passphrase, then store them in
`secrets/tooling.sops.env` alongside the provider credentials; see
[`secrets.md`](secrets.md).

Every OpenTofu command below is wrapped so credentials exist only inside that
child process. The file already uses the exact names OpenTofu reads, so there is
no mapping step and nothing to `unset` afterwards:
...
### If the server ever reports a public IPv6 address

Stop before applying. Do not add a `public_net` block to the imported
`hcloud_server`: on that path the provider can attempt an unplanned deletion of
the protected IPv4. IPv6 was removed once during adoption and must not be
re-enabled without equivalent firewall policy and testing.
...
./scripts/operator/with-secrets.sh --tooling -- \
  tofu -chdir=tofu import \
    ...
    hcloud_server.production "$SERVER_ID"

./scripts/operator/with-secrets.sh --tooling -- \
  tofu -chdir=tofu import \
    -var-file=production.tfvars \
    hcloud_primary_ip.production "$PRIMARY_IPV4_ID"
...
Plan files are encrypted too (`plan { enforced = true }`), so `tofu show` needs
the state passphrase just like `plan` and `apply`. The two `jq` checks must
return `true`. They are supplements, not substitutes,
...
No credentials outlive the window, because `with-secrets.sh` never exports them
into your shell. Remove the plan artifact:
